...
print.int_period <- function(x, ...) {
  ...
  cat(sprintf("<int_period> firstdate = %d, interval = %d\n", firstdate, interval))
...
as.list.int_period <- function(x, ...) {
  ...
  fd <- attr(x, "firstdate")
  dur <- attr(x, "interval")
  lapply(dat, new_int_period, interval = dur, firstdate = fd)
}
...
c.int_period <- function(..., recursive = FALSE, use.names = TRUE) {
  ...
  if (!all(vapply(firstdates, function(x) {is.null(x) || x == firstdate}, logical(1)))) {
    stop(
      "Unable to combine <int_period> objects with different `firstdate` attributes",
      call. = FALSE
    )
  }
...
new_int_period <- function(x = integer(), firstdate = integer(), interval = integer()) {
  structure(
    x,
    firstdate = firstdate,
    interval = interval,
