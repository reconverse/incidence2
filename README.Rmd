...
- **`rolling_average`** and **`remove_rolling`**: add or remove a rolling 
  average to/from an `incidence()` object.
...
- **`estimate_peak()`**: uses bootstrap to estimate the peak time (and related
  confidence interval) of a partially observed outbreak. We can use **`tidy()`**
  to generate nicer looking output.
...
By default this will not build local copies of the vignettes. If you would like
them installed, instead use:

```{r installvignette, eval=FALSE}
