...
complete_dates <- function(
    ...
    if (inherits(dates, "POSIXct") && !allow_POSIXct) {
        ...
        .stop(
            ...
            "Internally <POSIXct> objects are represented as seconds since the UNIX epoch and calling `complete_dates()` on an object of granularity can lead to significant memory usage. ",
            "If you are sure you wish to do this, please call again with the argument `allow_POSIXct` set to `TRUE`. ",
            "If this level of aggregation is not desired, consider recreating the <incidence> object using <Dates> for daily incidence. ",
