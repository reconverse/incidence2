...
test_that("accessors error when they should", {
  ...
  expect_error(get_firstdate("bob"))
})
...
test_that("accessors work", {
  ...
  expect_equal(get_firstdate(dat), as.Date("2020-12-26"))
})
# -------------------------------------------------------------------------
...
test_that("period output looks correct - character intervals", {
  ...
  dat_weeks <- table(as_period(dates, interval = "2 weeks"))
