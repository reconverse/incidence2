...
test_that("grouped month plot works", {
  ...
  expect_snapshot_plot("month_grouped", facet_plot(x, centre_dates=FALSE, n_breaks = 7, angle = 45))
})
...
test_that("grouped quarter plot works", {
  ...
  expect_snapshot_plot("quarterly_grouped", facet_plot(x, n_breaks = 6, angle = 45, facets = gender, fill = hospital))
})
