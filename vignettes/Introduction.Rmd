...
- The `incidence()` function:
  computes incidence from both linelist and pre-aggregated datasets across a range of date groupings.
- The returned object from `incidence()` is a subclass of [tibble](https://CRAN.R-project.org/package=tibble). This is compatible with [dplyr](https://dplyr.tidyverse.org/) for data manipulation (see `vignette("handling_incidence_objects")` for more details).
...
- Additional functions for handling incidence objects, including:
  ...
  * `as.data.frame.incidence2()` and `as_tibble.incidence2()` conversion methods.
...
For instance, we can compute incidence by gender and plot with both the `plot()` function for a single or the `facet_plot()` function for a multi-faceted plot across groups:
