...
i_weekly <- incidence(dat, date_index = date_of_onset, interval = "yrwk")
plot(i_weekly)
...
## year-monthly
i_monthly <- incidence(dat, date_index = date_of_onset, interval = "yrmon")
plot(i_monthly)
```
...
# a multi-facet plot
facet_plot(i_weekly_sex, fill = gender, n_breaks = 6, angle = 45)
```
...
inci <- incidence(dat, date_index = date_of_onset,
                     interval = "yrwk", groups = c(outcome, hospital, gender))
inci %>% 
  facet_plot(facets = hospital, fill = outcome, n_breaks = 6, nrow = 3, angle = 45)
```
