...
- Accessor functions: **`get_count_names()`**, **`get_dates_name()`**,
  **`get_date_index()`**, **`get_group_names()`**, **`get_interval()`**, 
...
plot(i_year, color = "white", n.breaks = 2)
```
...
# a multi-facet plot
facet_plot(i_weekly_sex, fill = gender, n.breaks = 5, angle = 45, color = "white")
```
