...
We try to make `incidence()` objects providing helper functions for both
manipulating and accessing data within the objects, and integration with
[tidyverse](https://www.tidyverse.org/) verbs.
...
* `get_count_vars()`, `get_date_vars()`, `get_date_group_vars()` and 
  `get_group_vars()` all return character vectors of the column names corresponding
