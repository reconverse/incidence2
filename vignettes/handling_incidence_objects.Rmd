...
## `rolling_average`
We can easily add a rolling average, across current and previous intervals, to
an `incidence()` object:
...
# to add a rolling average we pass an incidence object to `rolling_average`
ra <- 
  inci %>% 
...
# to remove a rolling average we pass the object to remove_rolling
ra %>% remove_rolling()
...
# individual plot would need regrouping if groups present
inci %>% 
  regroup %>% 
  rolling_average() %>% 
  plot(color = "white")
