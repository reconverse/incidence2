...
## `rolling_average`
We can easily add to an `incidence()` object a rolling average, across groupings, 
...
# remove the rolling average
remove_rolling(ra)

# plotting across the groups
ra %>% facet_plot(color = "white")

# we must remove groups if we want the overall rolling average
inci %>% 
  regroup %>% 
  rolling_average() %>% 
  plot(color = "white")
