...
**incidence2** is an R package that implements functions to compute, handle and
visualise *incidence* data. It aims to be intuitive to use for both interactive
data exploration and as part of more robust outbreak analytic pipelines.
...
The following sections give an overview of the package. They utilise two data
different data sets. Firstly, one from the
[outbreaks](https://cran.r-project.org/package=outbreaks) package containing a
synthetic linelist generated from a simulated Ebola Virus Disease (EVD)
outbreak. Secondly, one containing a pre-aggregated time-series of Covid cases,
tests, hospitalisations, and deaths for UK regions that was obtained using the [covidregionaldata](https://CRAN.R-project.org/package=covidregionaldata)
(extracted on 2021-06-03).
