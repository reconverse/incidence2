...
```{r}
library(incidence2)
library(outbreaks, include.only = c("ebola_sim_clean", "fluH7N9_china_2013"))
library(data.table, include.only = "frollmean")
library(ggplot2, include.only = c("geom_line", "aes"))
library(
    dplyr,
    include.only = c("select","filter", "mutate", "summarise", "reframe", "glimpse", "bind_rows"),
...
When working interactively it can feel a little onerous constantly having to
quote inputs for column names. To alleviate this we include the functions
`incidence_()` and `regroup_()` which both support
[tidy-select](https://dplyr.tidyverse.org/reference/dplyr_tidy_select.html)
semantics in their column arguments (i.e. `date_index`, `groups` and `counts`).

For now we have chosen to distinguish the functions via the post-fix underscore
and have a preference for the standard version for non-interactive (e.g.
programmatic usage). This could change over time if users feel having two
similar functions is confusing.
...
**Note:** The following sections utilise tidy-select semantics and hence use the
post-fix version of the incidence function (`incidence_()`)
...
weekly_incidence |> 
    ...
    class()
weekly_incidence |> 
    dplyr::filter(gender == "f", hospital == "Rokupa Hospital") |> 
    class()
...
weekly_incidence |> 
    dplyr::mutate(past = date_index - 999L) |> 
    class()
...
class(dplyr::bind_rows(weekly_incidence, weekly_incidence))
class(rbind(weekly_incidence[1:5, ], weekly_incidence[6:10, ]))
class(dplyr::bind_rows(weekly_incidence[1:5, ], weekly_incidence[6:10, ]))
```
...
- base: `split()`.
- [dplyr](https://cran.r-project.org/package=dplyr): `mutate()`, `summarise()` and `reframe()`.
...
weekly_incidence |>
    regroup_(hospital) |> 
    mutate(rolling_average = frollmean(count, n = 3L, align = "right")) |> 
